...
### [Private Version] - 2022.11

#### 优化与新内容

- 将谱面变速更改为滑动条，其区间为[0.5x,2x]
- 加入了全局音量调整
- 加入了Tips输入框
- 音符素材，打击特效整改（PhiPlayer）
- Autoplay更改为Demo
- 我不知道


### [Private Version] - 2022.10


#### 优化

- 允许连续上传zip文件
- 允许A.U.T.O Player实时切换
- 允许HyperJudge实时切换
- 谱面延时不再拥有上下限
- 将数值选择框更改为滑动条
- 负数Alpha视为0.001
- 允许不播放BGM（需暂停来生效）
- 等时音符——Tap为紫色（素材来源：[@Leekeqing]）
- 等时音符——Flick为绿色（素材来源：[@Leekeqing]）
- 调速选项为实时更改，用法：更改完重新开始
...
- 准确度显示器
- 开头过渡动画再延迟3秒
- 只有除Pureness外的音符数量之和少于3个时，判定为性歌，全连时的性歌将获得蓝“φ”，否则为白“φ”
- 全连时将获得“FC”Rank
- 新增定级选项，该选项只需要填写数字，结算时会通过定级作为计算RankingScore的依据
- 新增玩家名称，注意，录制手元时请不要提前写上，而是结算后退出全屏来签名
- 新增精选集，指定该歌曲隶属于何种精选集，该选项与模拟器作者[@lchz&#104;3473]的canvas水印绑定，通过“Copyright”开启
...
警告！该版本不会公开！本仓库的版本是远古版本，将会撤销素材的改动
