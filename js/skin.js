...
export default function() {
	...
	async function done() {
		console.log(files);
		var configFile;
		if (configFile = files.find(i => String(i.name).endsWith('config.txt'))) {
			var config = yaml2json(await stringify(configFile.buffer), /;?\r?\n/);
		} else if (configFile = files.find(i => String(i.name).endsWith('info.yml'))) {
			var config = yaml2json(await stringify(configFile.buffer));
		} else {
			return msgHandler.sendError('未找到config.txt或info.yml');
		}
		config = config || {};
		// const configFile = files.find(i => String(i.name).endsWith('config.txt'));
		// if (!configFile) return msgHandler.sendError('未找到config.txt');
		// const config = yaml2json(await stringify(configFile.buffer), /;?\r?\n/);
		// const info = files.find(i => String(i.name).endsWith('info.yml'));
		// if (!info) return msgHandler.sendError('未找到info.yml');
		// const infoJson = yaml2json(await stringify(info.buffer));
		...
		console.log(await stringify(configFile.buffer), config);
		console.log(entries);
	}
}
