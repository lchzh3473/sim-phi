...
function qwqdraw2() {
	...
	const xhr = new XMLHttpRequest();
	xhr.open("get", `src/LevelOver${difficulty < 0 ? 2 : difficulty}${hyperMode.checked ? "_v2" : ""}_v2.ogg`, true);
	xhr.responseType = 'arraybuffer';
	xhr.send();
	xhr.onload = async () => {
...
function qwqdraw3(statData) {
	...
	ctxos.drawImage(res["Ranks"][stat.rankStatus], 919*1.6 - qwq3, 100*3.3 - qwq3, qwq3 * 0.25, qwq3 * 0.25);
	...
	ctxos.fillText(statData.newBestStr, 1145.1-125, 500+50);
	ctxos.fillStyle = "#fff";
	ctxos.textAlign = "center";
	ctxos.font = "26px Saira";
	ctxos.fillText(statData.scoreBest, 1145.5+75, 500+50);
